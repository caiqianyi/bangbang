					...
					//	showColumns : true,
						...
						queryParams : function(params) {
							return {
								...
								flag:$("[name='flag'] option:selected").val(),
...
function edit(id,couponSurplus,couponCount) {
	if(parseInt(couponSurplus)<parseInt(couponCount)){
		layer.msg("该种类型的优惠券已经发放，不可编辑！！");
		return;
	}
	layer.open({
		type : 2,
...
function remove(id,couponSurplus) {
	if(parseInt(couponSurplus)>0){
		layer.msg("优惠券未发放完，不能删除！！")
		return;
	}
...
function sendout(couponId,couponSurplus,couponBalance,validity){
	if(parseInt(couponSurplus)==0){
		layer.msg("该种类型的优惠券已经发放完毕");
		return;
	}
	layer.open({
		type : 2,
